...
public:
    ...
    // -------------------------
    static constexpr uint32_t resolucion() {return TIMER1_resolucion;}
    ...
    // --------------------------
    /// Conectamos un reloj externo en el pin T1. Clock on falling edge.
    static void external_clock_falling_edge();

    /// Conectamos un reloj externo en el pin T1. Clock on rising edge.
    ...
    static uint16_t comparadorA();	/// Lectura del comparador A (OCR1A).
    static void comparadorA(uint16_t x);/// Escritura del comparador A.

    static uint16_t comparadorB();	/// Lectura del comparador B (OCR1B).
    static void comparadorB(uint16_t x);/// Escritura del comparador B (OCR1B).

    // ICR1 es una macro, asi que no se le puede llamar ICR1() a la función.
    // Con todo no me gusta llamarlo así. TODO: elegirle un buen nombre.
    static uint16_t registro_ICR1();	 /// Lectura del ICR1.
    static void registro_ICR1(uint16_t x);/// Escritura del ICR1.
...
inline uint16_t Timer1::comparadorA() 
{
    Interrupts_lock l;
    auto res = OCR1A;
...
inline void Timer1::comparadorA(uint16_t x)
{
    Interrupts_lock l;
    OCR1A = x;
}
...
inline uint16_t Timer1::comparadorB() 
{
    Interrupts_lock l;
    auto res = OCR1B;
...
inline void Timer1::comparadorB(uint16_t x)
{
    Interrupts_lock l;
    OCR1B = x;
}
...
inline uint16_t Timer1::registro_ICR1() 
{
    Interrupts_lock l;
    auto res = ICR1;
...
// Escribimos x en el comparador B
inline void Timer1::registro_ICR1(uint16_t x)
{
    Interrupts_lock l;
    ICR1 = x;
}
...
template<uint16_t periodo
	, uint16_t clock_frecuencia_en_hz>
...
public:
    ...
    // Recordar definir el valor de OCR1A/ICR1 elegido.
    /// Define el valor del TOP como el indicado en OCR1A.
    /// El TCNT va desde 0 hasta el valor escrito en OCR1A.
    static void top_comparadorA(const uint16_t& top0);

    /// Define el valor del TOP como el indicado en ICR1.
    /// El TCNT va desde 0 hasta el valor escrito en ICR1.
    static void top_ICR1(const uint16_t& top0);
...
inline void Timer1_CTC_mode::top_comparadorA(const uint16_t& top0)
{
    // 0100
...
inline void Timer1_CTC_mode::top_ICR1(const uint16_t& top0)
{
    // 1100
...
public:
    ...
    /// general, no llamar a esta función. Usar mejor el ICR1 o los valores
    /// fijos de top.
    static void top_comparadorA(uint16_t top0);

    /// El contador va desde 0 hasta el valor escrito en ICR1.
    static void top_ICR1(uint16_t top0);
...
//    static void toggle_pin_A_on_compare_match_pin_B_disconnected();
    ...
    /// En este modo si ponemos 300 en el comparador A y ICR1 = 1000, 
    /// el duty cycle es del 30% (suponiendo haber llamado a top_ICR1())
    static void pin_A_non_inverting_mode();

    /// Generamos en el pin A una señal PWM.
    /// En este modo si ponemos 300 en el comparador A y ICR1 = 1000, 
    /// el duty cycle es del 70% (suponiendo haber llamado a top_ICR1())
...
inline void Timer1_fast_PWM::top_comparadorA(uint16_t top0)
{
    // 1111
    atd::Register(TCCR1B).write_one_bit<WGM13, WGM12>();
    atd::Register(TCCR1A).write_one_bit<WGM11, WGM10>();
...
inline void Timer1_fast_PWM::top_ICR1(uint16_t top0)
{
    // 1110
    atd::Register(TCCR1B).write_one_bit <WGM13, WGM12>	();
    atd::Register(TCCR1A).write_one_bit <WGM11>		();
