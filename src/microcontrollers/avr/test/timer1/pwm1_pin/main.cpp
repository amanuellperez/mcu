...
// DONT_COMPILE(using PWM_pin1 = mcu::PWM1_pin<18>;)
using PWM_pin1 = mcu::PWM1_pin<15>;
using PWM_pin2 = mcu::PWM1_pin<16>;
...
void print(std::ostream& out, 
	   const mcu::timer1_::PWM_mode& pwm,
	   const mcu::Frequency& freq_clk)
{
    if (pwm.fast_mode){
...
void pwm_mode_test()
{
    ...
    mcu::Frequency freq_clk = 1_MHz;

    mcu::timer1_::PWM_mode pwm;

    for (mcu::Frequency fg = 20_Hz; fg < 100_Hz; fg += 1_Hz){
...
void generate_pwm_signal_pin_A()
{
    ...
    mcu::PWM_signal pwm{freq, static_cast<uint8_t>(duty_cycle)};
